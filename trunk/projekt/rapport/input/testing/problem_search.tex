...
It is generally the white box unit tests to provide code coverage which are described in this chapter, because those are the most important, since we are focused on the functionality of our system and therefore want as much of our code tested as possible.}
...
In order for D loop not to run there has been found enough problems or if the \vari{count} variable is at least equal to the number of tags selected.
...
The \cl{ProblemSearch} is the class which which is being tested, particularly the Search method in this test case.
