...
\item \sadb{Objects:} Problem, tag, category, \aclient[], \astaff[].
...
\item \sadb{Functions:} Add comment, reassign, change status, search database, create solution, attach solution and get staff \todolist{}.\fixme{make it right, when the functions are updated.} \fixme{staff must at least have the same functions as client}
\end{itemize}
...
\item\sadb{Functions:} Delete person, Add person, create department, set permissions, delete department, set priority, add category, delete category, set tag visibility, set category visibility, create tag, delete tag. \fixme{admin must at least have the same functions as client}
