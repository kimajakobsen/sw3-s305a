...
%\paragraph{A (Application domain)} People at the university or at another organization

%\paragraph{C (Conditions)} Only people associated with university should access our system, a webbased system, to-do-lists must be agile

%\paragraph{T (Technology)} Web based, DBMS

%\paragraph{O (Objects)} Problems, solutions, staffs, admins, clients, departments
...
\end{itemize}
\paragraph{R (Application domain)} This system will be applicable to different office environments aswel as helpdesk and IKT departments.

\paragraph{R (Conditions)} Users have varying technical experience. Users who are not trained to use our system will have to face a simple user-interface.

\paragraph{R (Technology)} In the development of our program we are going to use the programming language C# together with the Model-View-Controller framework that exists within the ASP.NET framework. We will set up a development server and use Team Foundation and Microsoft SQL server along with visual studio 2010.

The end result will be a web interface running on a webserver, with underlying SQL database.

\paragraph{R (Objects)} The system will contain a structure that can handle Problems, along with their solution, employees, supervisors, ordinary users and different department within an organization..

\paragraph{R (Responsibility) Our system is responsible for keeping track of all technically related problems within an organization. It is also responsible for distributing tasks amongst employees and supplying statistics on their progress to their supervisors.Finally it is also responsible for communication a solution to people who might be experiencing a particular problem.